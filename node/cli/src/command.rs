...
impl SubstrateCli for Cli {
	...
	fn load_spec(&self, id: &str) -> std::result::Result<Box<dyn sc_service::ChainSpec>, String> {
		...
		Ok(match id {
			// TODO: change configs
			"kusama" => Box::new(service::chain_spec::local_testnet_config()?),
			#[cfg(feature = "kusama-native")]
			"kusama-dev" => Box::new(service::chain_spec::development_config()?),
			#[cfg(feature = "kusama-native")]
			"kusama-local" => Box::new(service::chain_spec::local_testnet_config()?),
			#[cfg(feature = "kusama-native")]
			"kusama-staging" => Box::new(service::chain_spec::local_testnet_config()?),
			...
			path => {
				...
				let chain_spec = Box::new(service::KusamaChainSpec::from_json_file(path.clone())?)
					as Box<dyn service::ChainSpec>;

				// When `force_*` is given or the file name starts with the name of one of the known chains,
				// we use the chain spec for the specific chain.

				if self.run.force_kusama || chain_spec.is_kusama() {
	...
	fn native_runtime_version(spec: &Box<dyn service::ChainSpec>) -> &'static RuntimeVersion {
		return &service::node_template_runtime::VERSION;
		...
		Some(Subcommand::Benchmark(cmd)) => {
			...
			if chain_spec.is_kusama() {
				return Ok(runner.sync_run(|config| {
					cmd.run::<service::node_template_runtime::Block, service::KusamaExecutorDispatch>(
						config,
					)
