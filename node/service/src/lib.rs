...
#[cfg(feature = "full-node")]
pub use crate::overseer::{OverseerGen, OverseerGenArgs, RealOverseerGen};
...
pub use crate::chain_spec::ChainSpec as KusamaChainSpec;
	...
	ChainSelection: 'static + SelectChain<Block>,
{
	...
	let grandpa_hard_forks = grandpa_support::kusama_hard_forks();
	...
	OverseerGenerator: OverseerGen,
{
	...
	let disputes_enabled = chain_spec.is_kusama();

	let pvf_checker_enabled = !is_collator.is_collator() && false; // versi
	...
	if enable_grandpa {
		...
		let voting_rule = match grandpa_pause {
			Some((block, delay)) => {
				...
				builder.add(crate::grandpa_support::PauseAfterBlockFor(block, delay)).build()
			},
			None => builder.build(),
		};
...
> {
	...
	// 	return }
	chain_ops!(config, jaeger_agent, telemetry_worker_handle; node_template_runtime, KusamaExecutorDispatch, Kusama)
...
pub fn build_full(
	...
	#[cfg(feature = "kusama-native")]
	new_full::<node_template_runtime::RuntimeApi, KusamaExecutorDispatch, _>(
		config,
		is_collator,
		grandpa_pause,
		enable_beefy,
